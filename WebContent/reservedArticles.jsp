		...
		<tr width="100" class="cell">
			...
			<td width="900" class="cell" style="vertical-align: top" width="800">
				...
				style="font-size: 25px; letter-spacing: normal; position: relative; top: 0px;">
					<a
					href="selectedArticleManager.jsp?articleNo=<%=aBean.getArticleNo()%>&articleName=<%=aBean.getArticleName()%>&articleDesc=<%=aBean.getArticleDescription()%>&articleImg=<%=aBean.getArticleImage1WithParcedChars()%>&articleImg2=<%=aBean.getArticleImage2WithParcedChars()%>&articleImg3=<%=aBean.getArticleImage3WithParcedChars()%>&articlePrice=<%=aBean.getArticlePrice()%>"><%=aBean.getArticleName()%></a>
			</span><br> <span
				style="font-size: 15px; letter-spacing: normal; position: relative; top: 0px;"><%=aBean.getArticleDescription()%></span><br>
				<span> <img
					src="data:image/jpg;base64, <%=aBean.getBarcode()%>" alt="" /></span><br>

...
<form action="selectedArticleSeller.jsp" method="post">
	...
	<table id="tableId" border="1">
		...
		<tr width="100" class="cell">
			...
			<!-- 				<td width="20"></td>				 -->
			<td><img src=<%=aBean.getArticleImage1()%> alt="Image not found"
				onerror="this.src='images/carrosserie.jpg';" width="200"></img></td>
			<!-- 				<td width="20"></td> -->
			...
			<td width="900" class="cell" style="vertical-align: top" width="800">
				...
				style="font-size: 25px; letter-spacing: normal; position: relative; top: 0px;">
					<a
					href="selectedArticleSeller.jsp?articleNo=<%=aBean.getArticleNo()%>&articleName=<%=aBean.getArticleName()%>&articleDesc=<%=aBean.getArticleDescription()%>&articleImg=<%=aBean.getArticleImage1WithParcedChars()%>&articleImg2=<%=aBean.getArticleImage2WithParcedChars()%>&articleImg3=<%=aBean.getArticleImage3WithParcedChars()%>&articlePrice=<%=aBean.getArticlePrice()%>&articleCategory=<%=aBean.getArticleCategory()%>&articleOldNew=<%=aBean.getArticleOldNew()%>"><%=aBean.getArticleName()%></a>
			</span><br> <span

...
<form action="selectedArticleShopper.jsp" method="post">
	<%
...
<!-- 				<td width="20"></td>				 -->
				...
				<td width="900" class="cell" style="vertical-align: top" width="800"  >
					...
					style="font-size: 25px; letter-spacing: normal; position: relative; top: 0px;">
						<a href="selectMyReservedArticle.jsp?articleNo=<%=aBean.getArticleNo()%>&articleName=<%=aBean.getArticleName()%>&articleDesc=<%=aBean.getArticleDescription()%>&articleImg=<%=aBean.getArticleImage1WithParcedChars()%>&articleImg2=<%=aBean.getArticleImage2WithParcedChars()%>&articleImg3=<%=aBean.getArticleImage3WithParcedChars()%>&articlePrice=<%=aBean.getArticlePrice()%>"><%=aBean.getArticleName()%></a></span><br> <span
